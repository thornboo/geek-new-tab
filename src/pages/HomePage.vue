...
const title = computed(() => getCategoryByKey(categoryKey)?.label || 'Overview')

// CategoryView 业务逻辑
const { getSitesByCategory, addSite, updateSite, deleteSite } = useSiteManager()

const currentCategory = computed(() => getCategoryByKey(categoryKey))
const currentSites = computed(() => getSitesByCategory(categoryKey))
...
const handleFormSubmit = async (formData) => {
  if (formData.id && editingSite.value?.isCustom) {
    await updateSite(categoryKey, formData.id, formData)
  } else {
    await addSite(categoryKey, {
      name: formData.name,
      url: formData.url,
      desc: formData.desc
    })
...
const confirmDelete = async () => {
  if (deletingSite.value) {
    await deleteSite(categoryKey, deletingSite.value.id)
  }
...
<template>
  <AppLayout :title="title" active-page="category" :active-category="categoryKey">
