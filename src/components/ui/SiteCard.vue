...
<template>
  <div class="site-card group" @click="openSite">
    <!-- 右上角操作按钮 -->
    <div
      class="action-buttons absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 z-10"
    >
      <button
    ...
    <div class="mb-4">
      <div
        class="w-10 h-10 rounded border border-gray-700 bg-black flex-center group-hover:border-matrix transition-all"
      >
        <SiteIcon
          :icon="site.icon"
          :url="site.url"
    ...
    <div class="info flex-1 min-w-0">
      <h3
        class="name text-sm font-medium text-gray-200 mb-1 group-hover:text-matrix transition-all flex items-center gap-2"
      >
        {{ site.name }}
        <Icon v-if="site.isCustom" icon="mdi:circle-small" class="w-4 h-4 text-gray-600" />
      </h3>
      <p
        class="desc text-xs text-gray-500 line-clamp-2 h-8 leading-4 group-hover:text-gray-400 transition-colors"
      >
